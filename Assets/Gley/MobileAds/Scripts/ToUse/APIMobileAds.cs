...
namespace Gley.MobileAds.Scripts.ToUse
{
    public static class API
    {
        ...
        /// <param name="interstitialFailed">Callback called when interstitial fails to show</param>
        public static void ShowInterstitial(UnityAction interstitialClosed = null, UnityAction interstitialFailed = null)
        {
            if (IsInterstitialAvailable()) // Проверяем, доступна ли реклама
            {
                // Показать рекламу и передать коллбэк для закрытия
                MobileAdsManager.Instance.ShowInterstitial(() =>
                {
                    // Когда реклама закрыта, вызываем коллбэк закрытия
                    interstitialClosed?.Invoke();
                });
            }
            else
            {
                // Если реклама недоступна, вызываем коллбэк неудачи
