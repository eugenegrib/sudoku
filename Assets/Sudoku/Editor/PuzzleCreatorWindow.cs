...
namespace Sudoku.Editor
{
	public class PuzzleCreatorWindow : EditorWindow
	{
		...
		[MenuItem("Dotmob/LevelEditor", priority = 100)]
		public static void Init()
		{
			EditorWindow.GetWindow<PuzzleCreatorWindow>("@Dotmob Studio");
		}
		...
		private void Update()
		{
			if (worker != null)
			{
				...
				{
					bool cancelled = EditorUtility.DisplayCancelableProgressBar("Generating Sudoku Puzzles @DotmobStudio", string.Format("Creating puzzle {0} of {1}", numPuzzlesCreated + 1, numPuzzlesToCreate), (float)numPuzzlesCreated / (float)numPuzzlesToCreate);
