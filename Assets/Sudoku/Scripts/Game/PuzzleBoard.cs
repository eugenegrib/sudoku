...
namespace Sudoku.Scripts.Game
{
    public class PuzzleBoard : MonoBehaviour
    {
        ...
        [Space] [SerializeField] private Text noteOnOffText = null;
        ...
        {
            puzzleBoardCellPool =
                new ObjectPool(puzzleBoardCellPrefab.gameObject, 1, puzzleBoardCellContainer.transform);
            noteTextPool = new ObjectPool(noteTextPrefab.gameObject, 1,
                ObjectPool.CreatePoolContainer(transform, "note_pool"));
        ...
        public void SetNumber(int number)
        {
            ...
            if (number < 1 || number > activePuzzleData.boardSize)
            {
                Debug.LogErrorFormat(
                    "[PuzzleBoard] SetNumber(int number) : The given number is out of bounds for the board, number: {0}, bounds:[1,{1}]",
                    number, activePuzzleData.boardSize);
                return;
            }
        ...
        {
            var targetAlpha = isNotesOn ? 1.0f : 0.4f;
            StopAllCoroutines(); 
            StartCoroutine(FadeAlpha(targetAlpha));
            isNotesOn = !isNotesOn;
            noteOnOffText.text = !isNotesOn ? "OFF" : "ON";
            noteOnOffBkgImage.color = !isNotesOn ? noteOnColor : noteOffColor;
            noteOnOffText.color = !isNotesOn ? noteOnTextColor : noteOffTextColor;
        }
        ...
        private void SetBoxSelected(int boxRow, int boxCol, bool isSelected)
        {
            for (int row = 0; row < activePuzzleData.boxRows; row++)
            {
                ...
                {
                    SetCellSelected(boxRow * activePuzzleData.boxRows + row, boxCol * activePuzzleData.boxCols + col,
                        isSelected);
                }
            }
        }
        ...
        private void RemoveNotesForPlacedNumber(int row, int col, int number)
        {
            ...
            if (GameManager.Instance.RemoveNumbersFromNotes &&
                (!GameManager.Instance.ShowIncorrectNumbers ||
                 activePuzzleData.GetCellType(row, col) != PuzzleData.CellType.Incorrect))
            {
        ...
        {
            return Application.isPlaying && GameManager.Instance.ActivePuzzleData != null &&
                   ScreenManager.Instance.CurrentScreenId == "game";
        }
